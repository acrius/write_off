...
```
серверная часть приложения будет запущена. Во время запуска приложения осущевствляется связь с целевым серером, поэтом запуск может занять некоторе время. Если связь не установленна, то запуск всё-равно будет выполнен, просто на это потребуется больше времени. Далее необходимо открыть клиентскую часть приложения.
...
Статус активности отвечает за влияние акта на остатки по складу. На остатки влияют только акты со статусом "активен". Такие акты обозначаются в компоненте программы белым цветом. Если акт имет статус "не активен", то он обозначется тёмно зелёным цветом. Также акт может иметь статус "выгружен". данный акт обозначется синим цветом. Если акт был загружен в базу назначения, то он обозначается зелённым цветом. Для смена статуса с "не активен" на "активен" имеется кнопка на нижнем тулбаре, справа от кнопки "Добавить" или "Изменить". На кнопке имеется надпись "Активировать" или "Деактивировать", в зависимости от статуса акта. Подробнее о том, как назначить акту статус "выгружен" можно прочитать в пункте *Выгрузка актов во внешнюю базу*.
...
### Печать актов
Для печати акта предосмотрены две формы:
* М11 (Для актов перемещений)
* Ведомость учета ремонтных работ и использования запасных частей (Для актов списания)

Для печати акта в открытом форме в правом нижнем углу достаточно нажать кнопку печать.

**Важно!!! Перед печатью все внесенные в акт изменения должны быть сохранены!!!**
Если всё сделано правильно, то начнётся загрузка pdf файла с печатной формой акта. Далее файл необходимо открыть любой программой, работающей с pdf файлами.
