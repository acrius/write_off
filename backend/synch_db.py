...
from models import Act, OutAct, Subdivision, Storage, Remain, MainThing, DATABASES
...
class SynchDB:
	...
	def update_downloads(self):
		acts = self.in_session.query(Act).filter(
			and_(Act.upload_date >= datetime.now().date() - timedelta(weeks=10), Act.is_download == False))
		if acts.count() > 0:
			out_acts = self.out_session.query(OutAct).filter(OutAct.in_id.in_([act.id for act in acts])).order_by(OutAct.id)
			for out_act in out_acts:
				act = self.in_session.query(Act).get(out_act.in_id)
				act.is_download = out_act.is_download
				act.download_date = out_act.download_date
...
def update_downloads():
	try:
		synch = SynchDB(MAIN_ENGINE, OTSIDE_ENGINE)
		synch.update_downloads()
		print('Downloads update')
	except Exception as error:
		print('Downloads update fail')
		print(str(error))
